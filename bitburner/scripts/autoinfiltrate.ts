...
type MutableDocument = Document & {
  ...
  eventListeners?: Record<string, ListenerEntry[]>;
};
...
const wnd = eval('window') as MutableWindow;
const doc = wnd['document'] as MutableDocument;
...
const infiltrationGames: InfiltrationGame[] = [
  {
    ...
    play: function () {
      const data = state.game.data as string[] | undefined;
      if (!data || !data.length) {
  ...
  },
  {
    ...
    play: function (screen) {
      ...
      const code = Array.from(spanElements)
        .filter((span) => !span.getAttribute('style'))
      ...
      for (let y = 0; y < gridSize[1]; y++) {
        ...
        for (let x = 0; x < gridSize[0]; x++) {
          ...
          if (rows[index].children.length > 0) {
            gameData[y].push(true);
          } else gameData[y].push(false);
          index += 1;
        }
      }
    },
  ...
  },
  {
    ...
    init: function () {
      const data = state.game.data as boolean[][] | undefined;
      if (!data?.[0]) {
        return;
      }
      state.game.x = 0;
      state.game.y = 0;
      state.game.cols = data[0].length;
      state.game.dir = 1;
    },
    play: function () {
      let { data, x, y, cols, dir } = state.game as Required<
        Pick<GameState, 'data' | 'x' | 'y' | 'cols' | 'dir'>
      >;
  ...
  },
  {
    ...
    play: function (_screen) {
      const targets = state.game.data as Array<[number, number]> | undefined;
      const target = targets?.[0];
      let { x, y } = state.game as Required<Pick<GameState, 'x' | 'y'>>;
  ...
  },
  {
    ...
    init: function (screen) {
      ...
      const colors: Record<string, 'red' | 'white' | 'blue' | 'yellow'> = {
        red: 'red',
        white: 'white',
        blue: 'blue',
        'rgb(255, 193, 7)': 'yellow',
      };
      const wireColor: Record<'red' | 'white' | 'blue' | 'yellow', number[]> = {
        red: [],
        white: [],
        blue: [],
        yellow: [],
      ...
      const instructionNodes = Array.from(screen.children);
      const wiresData = instructionNodes.pop();
      instructionNodes.shift();
      const instructions = getLines(instructionNodes);
      ...
      for (let i = 0; i < instructions.length; i++) {
        ...
        if (-1 !== line.indexOf('cut all wires colored')) {
          ...
          const color = parts[2] as keyof typeof wireColor;

          if (!(color in wireColor)) {
            // should never happen.
            continue;
          }
    ...
    play: function () {
      const wire = state.game.data as number[] | undefined;
 ...
 * container.
 */
function getEl(parent: ParentNode | Element | string, selector?: string): NodeListOf<HTMLElement> {
  let prefix = ':scope';
  ...
  if ('string' === typeof parent) {
    ...
    if (!doc.querySelectorAll(prefix).length) {
      return doc.querySelectorAll(':not(*)') as NodeListOf<HTMLElement>;
    }
  }

  let selectorValue = (selector ?? '').split(',');
  selectorValue = selectorValue.map((item: string) => `${prefix} ${item}`);
  const selectorQuery = selectorValue.join(',');

  return parent.querySelectorAll(selectorQuery);
}

/**
 * Returns the first element with matching text content.
 */
function filterByText(elements: NodeListOf<HTMLElement>, text: string): HTMLElement | null {
 ...
 * @returns {string[]}
 */
function getLines(elements: ArrayLike<{ textContent: string | null }>): string[] {
  const lines: string[] = [];
  Array.from(elements).forEach((el) => lines.push(el.textContent ?? ''));
...
function wrapEventListeners() {
    ...
    doc.addEventListener = function (
      ...
      if ('keydown' === type) {
        ...
        handler = function (event: Event) {
          if (!('isTrusted' in event) || (event as KeyboardEvent).isTrusted) {
            return invoke(event);
          }
          ...
          for (const key in source) {
            ...
            } else if ('function' === typeof source[key]) {
              hackedEv[key] = (source[key] as (...innerArgs: unknown[]) => unknown).bind(event);
            } else {
              hackedEv[key] = source[key];
            }
          }

          Object.setPrototypeOf(hackedEv, KeyboardEvent.prototype);
          return invoke(hackedEv as unknown as Event);
        };

        if (typeof callback === 'function') {
          Object.assign(handler as object, callback);
        }
      }
      ...
      const self = this as MutableDocument;
      return self._addEventListener!(type, handler ? handler : callback, options);
    };
  }
  ...
  if (!doc._removeEventListener) {
    ...
    doc.removeEventListener = function (
      ...
      for (let i = 0; i < this.eventListeners[type].length; i++) {
        if (
          ...
          if (this.eventListeners[type][i].wrapped) {
            callback = this.eventListeners[type][i].wrapped as EventListenerOrEventListenerObject;
          }
      ...
      const self = this as MutableDocument;
      return self._removeEventListener!(type, callback, options);
    };
  }
}
