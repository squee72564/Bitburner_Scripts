...
function parseOptions(ns: NS): LoopOptions | null {
  ...
  if (flags.help) {
    ns.tprint('Usage: run scripts/hgw-loop.js [target] [--money 0.9] [--hack 0.1] [--epsilon 1]');
    return null;
  }
...
export async function main(ns: NS): Promise<void> {
  ...
  while (true) {
    ...
    if (maxMoney > 0 && money < maxMoney * moneyThreshold) {
      ...
      const neededGrow = ns.growthAnalyze(target, multiplier);
    ...
    const neededHack = ns.hackAnalyzeThreads(target, desiredHackAmount);
    ...
    if (maxMoney > 0 && moneyAfter < maxMoney) {
      const multiplier = maxMoney / Math.max(moneyAfter, 1);
      const neededGrow = ns.growthAnalyze(target, multiplier);
      const growThreads = capThreads(neededGrow, threadBudget);
      if (growThreads > 0) {
        const growSec = ns.growthAnalyzeSecurity(growThreads, target);
